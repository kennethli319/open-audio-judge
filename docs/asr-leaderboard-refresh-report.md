...
- Bundle status: `docs/asr-leaderboard-bundle-status.json`
...
- Load local Gemini secret before model runs: `source "${OPEN_AUDIO_JUDGE_GEMINI_ENV_FILE:?Set_OPEN_AUDIO_JUDGE_GEMINI_ENV_FILE_to_your_local_Gemini_environment_file}"`
...
| --- | --- | --- | --- |
| `preflight` | `cron_rehearsal_command`, `runtime_ready_check_command` | True | False |
...
| `verification` | `page_validation_command`, `report_bundle_check_command`, `freshness_check_command`, `commit_verification_command`, `cron_commit_verification_command` | False | False |
